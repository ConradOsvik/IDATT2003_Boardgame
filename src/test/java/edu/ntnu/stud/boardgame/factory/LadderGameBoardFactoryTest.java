...
class LadderGameBoardFactoryTest {
  ...
  void createBoard_validNames_returnsBoard() {
    ...
    for (String boardName : boardNames) {
      ...
      assertNotNull(
          board.getTile(board.getTiles().size() - 1), "Board should have an ending tile.");
    }
  }
  ...
  void createBoard_classicBoard_hasSpecificLadderAndSnake() {
    ...
    assertInstanceOf(LadderAction.class, tile1.getLandAction());
    assertEquals(
        classicBoard.getTile(40), ((LadderAction) tile1.getLandAction()).getDestinationTile());

    Tile tile24 = classicBoard.getTile(24);
    assertNotNull(tile24.getLandAction());
    assertInstanceOf(SnakeAction.class, tile24.getLandAction());
    assertEquals(
        classicBoard.getTile(5), ((SnakeAction) tile24.getLandAction()).getDestinationTile());
  }
