...
//
//    getNode().getChildren().addAll(controlPanel, canvasContainer);
//
//    calculateScaleFactors();
//    draw();
//  }
...
//
//    statusLabel = Label.builder().text("Status: Create a new game to begin").wrapText(true)
//    .build();
...
//            new ColorOption(Color.PURPLE, "Purple")).promptText("Select token color").disabled
//            (true)
//        .build();
...
//  }
//
//  private void drawLadder(GraphicsContext gc, int bottomTileId, int topTileId, int rows, int
//  cols) {
//    int[] bottomCoords = BoardCoordinateConverter.calculateTileCoordinatesFromId(bottomTileId,
//    rows,
//        cols);
...
//  }
//
//  public void drawPlayerToken(GraphicsContext gc, SlPlayer player, int tileId, int rows, int
//  cols) {
...
//
//  @Override
//  public void onGameEvent(_GameEvent event) {
...
import edu.ntnu.stud.boardgame.core.view.GameComponent;
import edu.ntnu.stud.boardgame.snakesandladders.controller.SlGameController;
import edu.ntnu.stud.boardgame.snakesandladders.view.components.BoardCanvas;
import javafx.scene.layout.HBox;
import javafx.scene.layout.Priority;
...
public class SlGameView extends GameComponent<HBox> {
  ...
  public SlGameView(SlGameController controller) {
    super(controller, new HBox());
    BoardCanvas canvas = new BoardCanvas(controller);

    HBox.setHgrow(canvas.getNode(), Priority.ALWAYS);

    getNode().getChildren().addAll(canvas.getNode());
  }
